...
object TpchRunner {
  def main(args: Array[String]): Unit = {
    ...
    //Configurations:
    var parquet_file_path = "/cloudgpfs/dataforge/ml-studio/yckj4506/data/dataset/benchmark-gluten/tpch_parquet_path"
    var tpchQueries = "tpch-queries/"
    if (args.length > 0) parquet_file_path = args(0)
    if (args.length > 1) tpchQueries = args(1)
    ...
    val fileLists = getListOfFiles(tpchQueries)
